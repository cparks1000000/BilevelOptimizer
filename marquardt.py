...
def LM(toy_input: Tensor, toy_target: Tensor, model: nn.Module, loss: Callable, n_iter: int = 10) -> List[Tensor]:
    ...
    for _ in range(n_iter):
        ...
        change = calc_change(model, loss_out, alpha)
...
def calc_change(model: nn.Module, loss_out: Tensor, alpha: float):
    ...
    gradients: Tuple[Tensor] = torch.autograd.grad(loss_out, to_list(model.parameters()), create_graph=True)
